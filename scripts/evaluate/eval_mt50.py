"""Real MT-50 evaluation for RCAR-VLA. Supports 4 ablation modes."""
import argparse
import json
import os
import time
...
from data.datasets.metaworld_dataset import MT50_TASKS, DIFFICULTY_TIERS
...
def rollout(model, env, task_name, lang_ids, device, max_steps=200,
    ...
    for step in range(max_steps):
        if chunk is None or chunk_idx >= model.horizon:
            rgbs = [render_view(env, "corner"), render_view(env, "corner2"), render_view(env, "gripperPOV")]
...
def evaluate(args):
    ...
    results = {}
    tier_succ = {t: 0 for t in DIFFICULTY_TIERS}
    tier_total = {t: 0 for t in DIFFICULTY_TIERS}
    overall_succ = 0
    overall_total = 0
    
    for task in MT50_TASKS:
        # Determine tier
        task_tier = "unknown"
        for tier, tasks in DIFFICULTY_TIERS.items():
            if task in tasks:
                task_tier = tier
                break
        ...
        try:
            ...
            results[task] = {"sr": 0.0, "n": 0, "error": str(e), "tier": task_tier}
            continue
        ...
        sr = succ / max(args.num_rollouts, 1)
        results[task] = {"sr": sr, "n": args.num_rollouts, "successes": succ, "tier": task_tier}
        overall_succ += succ
        overall_total += args.num_rollouts
        if task_tier in tier_succ:
            tier_succ[task_tier] += succ
            tier_total[task_tier] += args.num_rollouts
            
        print(f"  [{task:24s}] ({task_tier}) SR={sr*100:5.1f}%  ({succ}/{args.num_rollouts})")

    overall_sr = overall_succ / max(overall_total, 1)
    print(f"\n=== OVERALL MT-50 SR ({args.mode}) = {overall_sr*100:.1f}% ===")
    
    for tier in DIFFICULTY_TIERS:
        t_sr = tier_succ[tier] / max(tier_total[tier], 1)
        print(f"  - {tier.capitalize():10s}: {t_sr*100:5.1f}%")

    out = {
        "mode": args.mode, 
        "overall_sr": overall_sr,
        "tiered_sr": {t: tier_succ[t]/max(tier_total[t],1) for t in DIFFICULTY_TIERS},
        "per_task": results, 
        "args": vars(args)
    }
    Path(args.output_dir).mkdir(parents=True, exist_ok=True)
    fp = Path(args.output_dir) / f"mt50_{args.mode}.json"
    with open(fp, "w") as f:
...
if __name__ == "__main__":
    ...
    p.add_argument("--output_dir", default="outputs/eval/rcar_mt50")
    args = p.parse_args()
    evaluate(args)
